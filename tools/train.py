...
from datasets import ConcatDataset, build_vid
...
def main(cfg=cfg):
    ...
    parser.add_argument(
        "--config-file",
        default="./experiments/PTSEFormer_r50_demo.yaml",
        metavar="FILE",
    ...
    # 这里必须使用多帧，而不是单帧基线。
    # dataset_train = build_vid(image_set='train', cfg=cfg)  # 构建数据集，这里改为只使用demo数据集
    # dataset_val = build_vid(image_set='val', cfg=cfg)
    dataset_train = build_vidmulti(image_set='train', cfg=cfg)
